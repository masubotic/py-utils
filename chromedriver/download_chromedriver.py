...
def _get_cft_url(version: str, plat: str) -> str:
    ...
    data = json.loads(urllib.request.urlopen(CFT_LATEST_PATCH).read())
    ...
    data = json.loads(urllib.request.urlopen(CFT_KNOWN_GOOD).read())
    matches = [
...
def _get_legacy_url(version: str, plat: str) -> str:
    ...
    driver_version = urllib.request.urlopen(ver_url).read().decode().strip()
